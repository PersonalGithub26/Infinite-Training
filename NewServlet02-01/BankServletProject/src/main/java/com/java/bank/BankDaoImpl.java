...
public class BankDaoImpl implements BankDAO {
	...
	public Transaction searchTransation(int accountNo) throws ClassNotFoundException, SQLException
	{
		connection = ConnectionHelper.getConnection();
	...
	@Override
	public Transaction getMiniStatement(int accountNo) throws SQLException, ClassNotFoundException {
			
		   connection = ConnectionHelper.getConnection();
		    String cmd = "select * from trans where AccountNo=? order by TransDate desc limit 5";
		    pst = connection.prepareStatement(cmd);
		    pst.setInt(1, accountNo);
		    ResultSet rs = pst.executeQuery();
		    ...
		    if(rs.next())
		    {
		    	...
		    	transactions.setAccountNo(rs.getInt("accountNo"));
		    	transactions.setTransactionDate(rs.getTimestamp("transactionDate"));
		    	transactions.setTransactionType(rs.getString("transactionType"));
		    	transactions.setTransactionAmount(rs.getInt("transactionAmount"));
		    }
			/*
			 * List<Transaction> transactions = new ArrayList<>(); while (rs.next()) {
