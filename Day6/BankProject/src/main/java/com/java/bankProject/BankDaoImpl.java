...
public class BankDaoImpl implements BankDAO {
	...
	public String depositAccount(int accountNo, int depositAmount) throws ClassNotFoundException, SQLException {
		...
		if (bank!=null) {
			...
			String cmd2 = "Select status from Bank where AccountNo = ?";
	...
	public String withdrawAccount(int accountNo, int withdrawAmount) throws ClassNotFoundException, SQLException {
		...
		if (bank!=null) {
			...
			{
				return "This account is in inactive";
			}
		}
		return "Account No Not Found...";
	}
	
	public String checkStatus(int acNo) throws ClassNotFoundException, SQLException
	{
		connection = ConnectionHelper.getConnection();
		String cmd = "Select * from Bank where AccountNuber=?";
		pst.setInt(1, acNo);
		ResultSet rs = pst.executeQuery();
		rs.next();
